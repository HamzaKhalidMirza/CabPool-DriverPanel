...
export class LocationPickerMapComponent implements OnInit, AfterViewInit {
  ...
  @Input() lastModifiedField: string;
  @Input() center: any;
  @Output() centerEmitted = new EventEmitter<any>();
  @Output() confirmPcikupEmitted = new EventEmitter<any>();
  @Output() confirmDropoffEmitted = new EventEmitter<any>();
  pickup: boolean;
  dropOff: boolean;
  googleMapsSdk: any;
  ...
  ngOnInit() {
    if(this.lastModifiedField === 'Source') {
      this.pickup = true;
      this.dropOff = false;
    } else if(this.lastModifiedField === 'Dest') {
      this.dropOff = true;
      this.pickup = false;
    }
  }
  ...
  confirmPickup() {
    this.confirmPcikupEmitted.emit();
  }

  confirmDropoff() {
    this.confirmDropoffEmitted.emit();
  }
}
